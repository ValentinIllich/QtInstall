	...
	if( file.open(QIODevice::ReadOnly) )
	{
        ...
        while( file.bytesAvailable() )
		{
			...
			if( input.left(3)=="rf:" )
            {
                ...
                DatagramFileHandler::processUndo(input.mid(3));
            }
            if( input.left(3)=="sa:" )
            {
                dbgout(QString("    settings defaults='")+input.mid(3)+"'",1);
                QStringList list = input.mid(3).split(";");
                DatagramSettingsHandler::processSetting(list.at(0),"uninstalling",settingsAppAndOrgName,list.at(1));
            }
            if( input.left(3)=="rs:" )
            {
                dbgout(QString("    remove settings='")+input.mid(3)+"'",1);
                DatagramSettingsHandler::processSetting(input.mid(3),"uninstalling",settingsRemoveSettings,"");
            }
            if( input.left(3)=="sl:" )
            {
                dbgout(QString("    remove startup link='")+input.mid(3)+"'",1);
                DatagramLinksHandler::processLink(DatagramLinksHandler::eRemoveStartupLink,"uninstalling",input.mid(3),"",0);
            }
            if( input.left(3)=="dl:" )
            {
                dbgout(QString("    remove desktop link='")+input.mid(3)+"'",1);
                DatagramLinksHandler::processLink(DatagramLinksHandler::eRemoveDesktopLink,"uninstalling",input.mid(3),"",0);
            }
